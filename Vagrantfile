...
Vagrant.configure(2) do |config|
  ...
  config.vm.synced_folder _conf['synced_folder'],
      _conf['document_root'], create: true, type: "smb", smb_username: _conf['smb_username'], smb_password:  _conf['smb_password'], mount_options: ["mfsymlinks,dir_mode=0775,file_mode=0664"]

  # config.vm.synced_folder _conf['synced_folder'],
  #     _conf['document_root'], :create => "true", type: "smb", smb_username: _conf['smb_username'], smb_password:  _conf['smb_password'], :mount_options => ['mfsymlinks','dmode=755', 'fmode=644']
  ...
  config.vm.provider "hyperv" do |h|
    ...
    h.linked_clone = true
    # h.name = _conf['hostname']
    h.memory = _conf['memory'].to_i
    h.cpus = _conf['cpus'].to_i
    h.vmname = "pae_dev"
